...
template<class graph>
void transposePartition(graph* G, partitionData* TD, 
                        intE* updateBinAddrOffset, intE* destIdBinAddrOffset)
{
//    graph* G = TD->G;
    ...
    for (intV i=0; i<G->numBins; i++)
        updateBinAddrOffset[i] = GSort->VI[i+1]; // 产生的更新消息数�?
...
/* 将part的边全部写入destIdBins
 * - 如果连续出现的话, 则目标节点要按位或上MAX_NEG*/
template<class graph>
#ifdef WEIGHTED
void writeDestIds(graph* G, partitionData* TD, intV** destIdBins, unsigned int** weightBins, intE* destIdBinPointers)
#else
void writeDestIds(graph* G, partitionData* TD, 
                intV** destIdBins, intE* destIdBinPointers)
#endif
{
    intV destId = 0;
    intV destBin = 0;
...
/* 创建一个numRows x numCols的二维数组，一般是bin数据结构的控制结�? */
